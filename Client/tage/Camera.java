...
public class Camera
{
	...
	public void yaw(float amount) {
		...
		float vy = getV().y();
		float vz = getV().z();
		setU(getU().rotateAxis(amount, vx, vy, vz));
		setN(getN().rotateAxis(amount, vx, vy, vz));
	}
	...
	public void pitch(float amount) {
		...
		float uy = getU().y();
		float uz = getU().z();
		setV(getV().rotateAxis(amount, ux, uy, uz));
		setN(getN().rotateAxis(amount, ux, uy, uz));
	}
	...
	public void roll(float amount) {
		...
		float ny = getN().y();
		float nz = getN().z();
		setU(getU().rotateAxis(amount, nx, ny, nz));
		setV(getV().rotateAxis(amount, nx, ny, nz));
	}
